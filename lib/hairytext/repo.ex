...
defmodule HT.Repo do
  ...
  def start_link(db_folder) do
    #IO.inspect(db_folder, label: :HTRepostart_link)
		state = for schema <- schemas(), into: %{} do
  ...
  def init(state) do
    #IO.inspect(state, label: :HTRepo_init)
    {:ok, state}
  end

  def all(thing) do
    #IO.inspect(thing, label: :HTRepo_all)
  ...
  def get!(thing, id) when is_atom(thing) do
	...
	def insert(%_type{} = thing) when is_struct(thing) do
		IO.inspect(thing, label: :Repo_insert)
  ...
  def pids() do
	...
	def update(%_type{} = query) when is_struct(query) do
		# IO.inspect(query, label: Repo_update)
		IO.inspect(query.data, label: Repo_update_data)
		IO.inspect(query.changes, label: Repo_update_chgs)
  ...
  def handle_call({:all, %type{} = thing}, from, state) do
	...
	def handle_call({:insert, %Ecto.Changeset{valid?: true} = changeset}, _from, state) do
		with %type{} = value = changeset.data do
			IO.inspect(type, label: :inserttype)
			IO.inspect(value, label: :insertdata)
			IO.inspect(changeset.changes, label: :insertchgs)
	...
	"""
  def handle_call({:insert, %type{} = value}, _from, state) do
		# IO.inspect {type,value}, label: :handle_call_insert
		v2 = value 
			|> unnil(:id, &uuid/0)
			|> unnil(:inserted_at, &DateTime.utc_now/0)
			|> unnil(:updated_at, &DateTime.utc_now/0)
		...
		id = Map.get(v2, :id)
		handle = Map.get(state, type)
		item = {id, v2 |> Map.drop([:id])}
		:dets.insert(handle, item)
    {:reply, item, state}
  end
	"""
  ...
  def handle_call(:pids, _from, state) do
	...
	def handle_call({:update, %Ecto.Changeset{valid?: true} = changeset}, _from, state) do
      ...
      IO.inspect(changeset, label: :chgs)
			handle = Map.get(state, type)
			id = Map.get(query, :id)
			data = get_by_id!(type, handle, id)
				|> Map.put :updated_at, DateTime.utc_now()
      data2 = Enum.reduce(changeset.changes, data, 
        fn {k, v}, acc -> 
          IO.inspect({k,v}, label: :upd)
          IO.inspect(acc, label: :upd)
          Map.put(acc, k, v) 
        end
      )
			data3 = data2 |> Map.drop([:__meta__, :__schema__, :__struct__])
      IO.inspect({id, data3}, label: :Repo_update)
	...
	defp get_by_id!(type, handle, id) do
		data = case :dets.match(handle, {id, :'$1'}) do
			[] -> []
			other ->
				other
				|> hd
